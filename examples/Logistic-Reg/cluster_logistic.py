...
for ACCEPT_RATE in [1., .999, .99, .975, .95, .925, .9, .8, .75, .7, .6, .5, .4, .3, .2, .1]:
    print("\n\n--------------------")
    ...
    params, train_accuracy, train_losses, test_accuracy, test_losses, key = \
        train_loop(
        key, N_EPOCHS, NUM_LAYERS, HIDDEN_SIZE, NUM_CLASSES, BATCH_SIZE, NUM_BATCH, LEARNING_RATE, WDECAY, PATIENCE, COOLDOWN, FACTOR, RTOL, ACCUMULATION_SIZE, LEARNING_RATE_MIN, prior_simulator, data_simulator, discrepancy, 
        true_data = TRUE_DATA, X_train = None, y_train = None, X_test = X_test, y_test =  y_test, N_POINTS_TRAIN = N_POINTS_TRAIN, N_POINTS_TEST = N_POINTS_TEST, epsilon = EPSILON_STAR, verbose = True)
    ...
    name = "./pickle/Logistic_Reg_influent_{}_sigma0_{}_acc_{:.2}_eps_{:.5}.xz".format(N_VAR, SIGMA0, ACCEPT_RATE, EPSILON_STAR)

    with lzma.open(name, "wb") as f:
        pickle.dump(pickle_dico, f)
    print("Data saved in ", name)

    title = "Normal w/ known std\nsigma0 = {} mu = {}\nalpha = {:.2%}, eps = {:.3} accuracy = {:.2%}".format(SIGMA0, TRUE_BETA, ACCEPT_RATE, EPSILON_STAR, test_accuracy[-1])

    name_plot = "./fig/Logistic_Reg_influent_{}_sigma0_{}_acc_{:.2}_eps_{:.5}.png".format(N_VAR, SIGMA0, ACCEPT_RATE, EPSILON_STAR)
