...
def get_potus_model(stan_data_json):
    ...
    with pm.Model() as m2:
        ...
        rho_e_bias = pm.TruncatedNormal("rho_e_bias", mu=0.7, sigma=0.1, lower=0.0, upper=1.0)
        ...
        # Innovations
        innovs = pm.math.dot(cholesky_ss_cov_mu_b_walk, raw_mu_b[:, :-1])
        ...
        sigma_rho =  np_data["sigma_e_bias"]/pm.math.sqrt(1 - (rho_e_bias**2))
        ...
        e_bias = pm.Deterministic(
            "e_bias", pm.math.dot(A_inv, sigma_vec * raw_e_bias + mus)
        )    
