...
def compute_wasserstein(samples_1: np.ndarray, samples_2: np.ndarray) -> float:
    """
    ...
    return stats.wasserstein_distance(
        np.asarray(samples_1).flatten(), np.asarray(samples_2).flatten()
    )
...
def compute_mse_mean_std(
    ...
    return {"mse_mean": float(mse_mean), "mse_std": float(mse_std)}
...
def compute_js_divergence(samples1: np.ndarray, samples2: np.ndarray) -> float:
    ...
    min_val, max_val = min(np.min(samples1), np.min(samples2)), max(
        np.max(samples1), np.max(samples2)
    )
...
def compute_coverage_metrics(
    approx_samples: np.ndarray, true_value: float
) -> Dict[str, float]:
...
def run_metrics_suite(
    ...
    for approx_name, approx_samples in approx_samples_dict.items():
        ...
        if "wasserstein" in metrics_to_run:
            results["wasserstein_distance"] = compute_wasserstein(
                true_samples, approx_samples
            )
