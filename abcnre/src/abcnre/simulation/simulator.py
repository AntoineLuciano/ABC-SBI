...
class ABCSimulator:
    ...
    def _init_summary_stats(self):
        ...
        if not self.trained_summary_stats:
            self.summary_stat_fn = getattr(self.model, "predefined_summary_stat_fn", None)
    ...
    def _init_epsilon(self):
        ...
        if self.quantile_distance is not None and not self._epsilon_provided:
            ...
            if self.config.get("verbose", False):
                logger.info(f"Computing epsilon for {self.quantile_distance:.1%} quantile...")
            if self.quantile_distance == 1.0:
                ...
                logger.info("Setting epsilon to infinity (maximum distance).")
            # Use a fixed key for this internal, automatic setup
                ...
                if self.config.get("verbose", False):
                    logger.info(f"Computed epsilon = {self.epsilon:.6f}")
        elif self._epsilon_provided and self.config.get("verbose", False):
            logger.info(f"Using provided epsilon = {self.epsilon:.6f}")
    ...
    def _init_sampler(self):
        ...
        observed_summary_stats_for_sampler = self.observed_summary_stats
        ...
        if self.observed_data is None and self.epsilon == jnp.inf:
            ...
            if self.config.get("verbose", False):
                logger.info("Created temporary observed data for prior sampling mode")
    ...
    def train_summary_network(
        ...
        if not hasattr(self.model, "summary_stat_fn") or override_model_summary_stats:
            if nn_config.training.verbose:
                logger.info("Updating model's summary statistics function...")
        ...
        if nn_config.training.verbose:
            logger.info(f"Summary statistics learned and updated successfully!\n    - Original data dimension: {self.model.data_shape}\n    - Learned summary statistics dimension: {self.observed_summary_stats.shape if self.observed_summary_stats is not None else 'None'}")
