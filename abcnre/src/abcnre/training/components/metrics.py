...
class TrainingMetrics:
    ...
    def __init__(self, task_type: str):
        ...
        """
        if task_type not in ["classifier", "summary_learner"]:
    ...
    def add_epoch_metrics(self, epoch_data: Dict[str, Any]):
        ...
        if self.task_type == "classifier":
            ...
            if "train_accuracy" in epoch_data:
                self.metrics["train_accuracy"].append(float(epoch_data["train_accuracy"]))
    ...
    def get_final_summary(self, total_time: float) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_simulations": final_simulations if final_simulations is not None else 0,
    ...
    def get_best_epoch(self, metric_name: str, mode: str = "min") -> Dict[str, Any]:
        ...
        return {
            ...
            "improvement_from_start": float(abs(values[best_idx] - values[0])) if len(values) > 0 else 0
        }
...
class TrainingLogger:
    ...
    def log_training_start(self, task_type: str, num_epochs: int, batch_size: int, n_batch: int):
        """Log training start information."""
        if self.verbose:
            logger.info("="*60)
            logger.info(f"Starting {task_type} training")
            logger.info(f"Epochs: {num_epochs}, Batch size: {batch_size}, Batches/epoch: {n_batch}")
            logger.info("="*60)
    ...
    def log_lr_reduction(self, old_lr: float, new_lr: float):
        ...
        if self.verbose:
            ...
            logger.info(f"Learning rate reduced: {old_lr:.2e} -> {new_lr:.2e} ({reduction_factor:.2f}x)")
    ...
    def log_training_completion(self, metrics: TrainingMetrics, total_time: float):
        ...
        if best_val_loss:
            logger.info(f"Best val loss: {best_val_loss['value']:.6f} (epoch {best_val_loss['epoch']})")
    ...
    def log_metrics_summary(self, metrics: TrainingMetrics):
        ...
        for metric_name in ["train_loss", "val_loss"]:
            ...
            if stats:
                logger.info(f"   {metric_name}: min={stats['min']:.6f}, max={stats['max']:.6f}, mean={stats['mean']:.6f}")
        
        if self.task_type == "classifier":
            for metric_name in ["train_accuracy", "val_accuracy"]:
                stats = metrics.get_summary_stats(metric_name)
                if stats:
                    logger.info(f"   {metric_name}: min={stats['min']:.3f}, max={stats['max']:.3f}, mean={stats['mean']:.3f}")


def evaluate_batch_metrics(task_type: str, params, batch_data, network, loss_fn) -> Dict[str, float]:
    """
...
def compute_metrics_delta(metrics: TrainingMetrics, metric_name: str, window: int = 10) -> float:
    """
    ...
    older_avg = sum(history[-2*window:-window]) / window if len(history) >= 2*window else history[0]
    
    return recent_avg - older_avg


def is_training_stagnant(metrics: TrainingMetrics, patience: int = 20, threshold: float = 1e-6) -> bool:
    """
